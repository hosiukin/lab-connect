    ...
    if CONFIG_FILE.exists():
        try:
            ...
            if isinstance(stored, dict):
                ...
                # Migrate forwards that are missing bind_host
                for forward in config.get("forwards", []):
                    if isinstance(forward, dict) and "bind_host" not in forward:
                        forward["bind_host"] = "0.0.0.0"
...
def validate_config(raw: dict) -> dict:
    ...
    is_reverse = config["direction"] == "reverse"
    allowed_text = (
        "profile_name",
        "jump_host",
        "jump_user",
    ...
    for index, item in enumerate(raw_forwards):
        ...
        bind_host = str(item.get("bind_host", "0.0.0.0")).strip()
        ...
        if not bind_host or any(char.isspace() for char in bind_host):
            raise ValueError(f"Forward '{name}' has an invalid bind host")
...
def ssh_config_text(config: dict) -> str:
    ...
    if direction == "reverse":
        header = "# Managed by Lab Connect (REVERSE mode). Edit through the Lab Connect UI."
    else:
        header = "# Managed by Lab Connect. Edit through the Lab Connect UI."
    return f"""{header}
Host {jump_alias}
...
def control_path(config: dict) -> Path:
    ...
    if config.get("direction") == "reverse":
        suffix = "-rev.ctl" if not IS_WINDOWS else "-rev.pid"
    else:
...
def tunnel_command(config: dict) -> list[str]:
    ...
    if config.get("direction") == "reverse":
        for forward in config["forwards"]:
            bind = forward.get("bind_host", "0.0.0.0")
            command.extend(
                [
                    "-R",
                    f"{bind}:{forward['remote_port']}:{forward['remote_host']}:{forward['local_port']}",
                ]
            )
        command.append(jump_alias)
    else:
        for forward in config["forwards"]:
            command.extend(
                [
...
def tunnel_status(config: dict) -> dict:
    ...
    is_reverse = config.get("direction") == "reverse"
    endpoints = [
        {
            **forward,
            "endpoint": f"{config['jump_host']}:{forward['remote_port']}"
            if is_reverse
            else f"127.0.0.1:{forward['local_port']}",
        }
        for forward in config["forwards"]
    ]
    if IS_WINDOWS:
...
def start_tunnel(config: dict) -> dict:
    ...
    if IS_WINDOWS:
        ...
        jump_alias, target_alias = ssh_aliases(config)
        is_rev = config.get("direction") == "reverse"
        args = [
            executable("ssh"),
            "-fNT",
            "-M",
...
def open_client(config: dict, forward_index: int = 0) -> dict:
    ...
    is_reverse = config.get("direction") == "reverse"
    if is_reverse:
        address = f"{config['jump_host']}:{forward['remote_port']}"
    else:
        ...
        checks.append(
            {
                "name": "Port forwards",
                "ok": True,
                "output": "No port forwards are configured.",
            }
        )
    else:
        for forward in config["forwards"]:
