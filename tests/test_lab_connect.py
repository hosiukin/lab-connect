...
class ConfigTests(unittest.TestCase):
    def test_validate_config(self):
        ...
        self.assertEqual(config["direction"], "forward")
    ...
    def test_reverse_mode_target_optional(self):
        config = lab_connect.validate_config(
            {
                "profile_name": "home-pc",
                "jump_host": "10.30.24.1",
                "jump_user": "home-user",
                "direction": "reverse",
                "forwards": [
                    {
        ...
        self.assertEqual(config["direction"], "reverse")
        # target fields should have default empty values in reverse mode
        self.assertEqual(config["target_host"], "")
        self.assertEqual(config["target_user"], "")

    def test_reverse_mode_rejects_bad_bind_host(self):
    ...
    def test_ssh_config_reverse_has_note(self):
        config = lab_connect.validate_config(
            {
                "profile_name": "home-pc",
                "jump_host": "10.30.24.1",
    ...
    def test_reverse_tunnel_command_uses_R_flag_and_jump_alias(self):
        config = lab_connect.validate_config(
            {
                "profile_name": "home-pc",
                "jump_host": "10.30.24.1",
        ...
        self.assertIn("0.0.0.0:13389:127.0.0.1:3389", command)
        self.assertEqual(command[-1], "home-pc-jump")
    ...
    def test_load_config_migrates_legacy_service(self):
        ...
        self.assertEqual(config["forwards"][0].get("bind_host"), "0.0.0.0")

    def test_load_config_migrates_missing_direction(self):
        with tempfile.TemporaryDirectory() as folder:
            config_file = Path(folder) / "config.json"
            config_file.write_text(
                """
                {
                  "profile_name": "old-config",
                  "jump_host": "10.0.0.1",
                  "jump_user": "user",
                  "target_host": "10.0.0.2",
                  "target_user": "target",
    ...
    def test_load_config_migrates_forwards_missing_bind_host(self):
        with tempfile.TemporaryDirectory() as folder:
            config_file = Path(folder) / "config.json"
            config_file.write_text(
                """
                {
                  "profile_name": "old-forwards",
                  "jump_host": "10.0.0.1",
                  "jump_user": "user",
                  "target_host": "10.0.0.2",
                  "target_user": "target",
            ...
            with mock.patch.object(lab_connect, "CONFIG_FILE", config_file):
                config = lab_connect.load_config()
        self.assertEqual(config["forwards"][0]["bind_host"], "0.0.0.0")
    ...
    def test_reverse_control_path_has_rev_suffix(self):
        config = lab_connect.validate_config(
            {
                "profile_name": "home-pc",
                "jump_host": "10.30.24.1",
        ...
        path = lab_connect.control_path(config)
        self.assertIn("-rev", str(path))

    def test_forward_control_path_no_rev_suffix(self):
        config = lab_connect.validate_config(
            {
                "profile_name": "spark",
                "jump_host": "192.0.2.10",
        ...
        path = lab_connect.control_path(config)
        self.assertNotIn("-rev", str(path))

    def test_reverse_tunnel_status_endpoints_use_jump_host(self):
        config = lab_connect.validate_config(
            {
                "profile_name": "home-pc",
                "jump_host": "10.30.24.1",
        ...
        endpoints = config["forwards"]
        self.assertEqual(endpoints[0]["local_port"], 3389)
        self.assertEqual(endpoints[0]["remote_port"], 13389)
        self.assertEqual(endpoints[0]["bind_host"], "0.0.0.0")

    def test_reverse_open_client_uses_jump_host_address(self):
        config = lab_connect.validate_config(
            {
                "profile_name": "home-pc",
                "jump_host": "10.30.24.1",
        ...
        # open_client in reverse mode should generate jump_host:remote_port address
        with mock.patch("lab_connect.platform") as mock_platform:
            mock_platform.system.return_value = "Darwin"
            result = lab_connect.open_client(config, 0)
        self.assertTrue(result["ok"])
        self.assertIn("10.30.24.1:13389", result["output"])
